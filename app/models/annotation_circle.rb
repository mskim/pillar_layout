...
class AnnotationCircle < ApplicationRecord
  ...
  def to_svg
    ...
    s += "<circle cx='#{x}' cy='#{y}' r='20' stroke='crimson' stroke-width='4' fill='white' fill-opacity='0' class='draggable' data-circle-id='#{id}' data-user-id='#{user_id}' data-move-draggable-url='#{move_draggable_annotation_circle_path(self)}' />"
    s += "<defs>
      <clipPath id='avatarCircle#{id}'>
        <circle cx='#{x + 25}' cy='#{y - 25}' r='10' fill='#FFFFFF' data-target='draggable.mask' data-circle-id='#{id}' />
      </clipPath>
    </defs>"
    s += "<image xlink:href='#{Rails.application.routes.url_helpers.rails_blob_path(user.avatar, only_path: true)  if user.avatar.attached?}' x='#{x + 15}' y='#{y - 35}' width='20' height='20' clip-path='url(#avatarCircle#{id})' class='avatar' data-target='draggable.avatar' data-circle-id='#{id}' />\n"
    s += "<a xlink:href='/annotation_circles/#{id}/delete_it'>
    <g transform='translate(#{x - 24}, #{y - 24})' data-target='draggable.closeIcon' data-circle-id='#{id}' data-user-id='#{user_id}'>
