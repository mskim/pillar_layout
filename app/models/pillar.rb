...
class Pillar < ApplicationRecord
  ...
  def add_right_drop(drop_column, starting_index)
    return if drop_column >= column - 1
    return if has_drop_article?
    update(has_drop_article: true)
    # update all existing articles column
    new_column = column - drop_column
    ...
    h = {}
    h[:kind] = "right_drop"
    h[:grid_x] = column - drop_column
    h[:grid_y] = working_articles[starting_index].grid_y
    h[:column] = drop_column
    h[:row]    = row - h[:grid_y]
  ...
  # if current article is not the top article, lock all article above the currnt one.
  def add_left_drop(drop_column, starting_index)
    return if drop_column >= column - 1
    return if has_drop_article?
    update(has_drop_article: true)
    new_column = column - drop_column
    ...
    working_articles.each do |w|
      w.update(grid_x:drop_column, column:new_column)
      w.generate_pdf_with_time_stamp
    end
    h          = {}
    h[:kind]   = "left_drop"
